...
### Reporting extras
**`/reports` is one page that answers two different questions.** `reports.view` gets you in; `reports.view-all` decides *whose* numbers you see. `ReportController` resolves a single `?User $scope` up front — null for the Super Admin and anyone holding `reports.view-all` (the Manager), the viewer otherwise — and threads it through every tile, chart and list, so no part of the page can quietly answer a wider question than the rest. A scoped report also drops the three cards that rank colleagues against each other (workload by person, workload by team, top performers) and narrows the activity trail to the viewer's own, and the page renders as "My Reports" off a `scoped` prop. When adding a figure, take the scope — don't reach for a bare `Task::query()`.
...
What the `employee` floor deliberately *omits* is as much of the design as what it includes (`RolePermissionSeeder::employeePermissions()`): no `*.view-all`, no assign, no delete, and **no `teams.view`** — the Teams panel is a management surface, and the sidebar hides it because the permission is absent rather than because `navigation.ts` says so. A team lead gets it back through their responsibility bundle. `reports.view` without `reports.view-all` is a *personal* report, not a smaller copy of the workspace one — see **Reporting extras**. Re-run `db:seed` (or just `RolePermissionSeeder`) after changing that list; role permissions are synced, so a removed slug is actually revoked.
...
Permission checks gate *routes*; `scopeVisibleTo(User)` gates *rows*. `Project::visibleTo()` returns everything for admins and anyone holding `projects.view-all`, otherwise only projects the user owns or is a member of. `Task::visibleTo()` is the same idea via `tasks.view-all`, assignee, reporter, team routing or **project ownership**. Always apply the scope in index/show queries — permission middleware alone does not scope data.

**Plain project membership does not reveal a project's tasks.** It used to, which made an employee's task list the entire board of every project they sat on. A developer now sees the work that is actually theirs — assigned to them, raised by them, or routed to a team they belong to (that last one is what keeps unassigned team work pickable). Anyone who needs the whole board holds `tasks.view-all`, which the Manager role and the reporting bundles carry. Being the project *owner* still carries accountability for the work inside it.

Because `TaskPolicy::inScope()` funnels through the same scope, narrowing it narrows reads *and* writes everywhere at once — there is no second list to keep in step. Pinned by `tests/Feature/Tasks/EmployeeScopeTest.php`.
