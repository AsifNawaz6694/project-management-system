...
import { ArrowLeft, CalendarDays, CheckCircle2, Circle, Download, File, ListTodo, Pencil, Plus, Trash2, UploadCloud, Users } from 'lucide-react';
...
type Tab = 'overview' | 'milestones' | 'team' | 'discussion' | 'files' | 'activity';

export default function ProjectShow({ project, activities, milestoneStats, comments, canUpload }: ProjectShowProps) {
            ...
            <div className="flex w-full flex-1 flex-col gap-6 p-4 md:p-6">
                ...
                <SoftCard className="overflow-visible">
                    <div className={cn('relative h-44 overflow-hidden rounded-t-2xl bg-gradient-to-br', gradient)}>
                        <div className="absolute -top-24 -right-24 size-80 rounded-full bg-white/15 blur-3xl" />
                        <div className="absolute -bottom-12 left-1/4 size-60 rounded-full bg-white/15 blur-3xl" />
                        <div
                            className="absolute inset-0"
                            style={{ backgroundImage: 'radial-gradient(rgba(255,255,255,0.18) 1px, transparent 1px)', backgroundSize: '28px 28px' }}
                        />
                        <div className="relative flex items-start justify-between p-6">
                            <div className="flex flex-col items-start gap-3">
                                <span className="inline-flex items-center gap-1.5 rounded-full bg-white/20 px-3 py-1 text-[11px] font-bold tracking-[0.14em] text-white uppercase ring-1 ring-white/30 backdrop-blur">
                            ...
                            <div className="flex items-center gap-2">
                                <Button
                                    ...
                                    variant="ghost"
                                    size="sm"
                                    className="gap-1.5 bg-white/15 text-white ring-1 ring-white/25 hover:bg-white/25"
                                >
                                    <Link href={route('sprints.backlog', project.slug)}>
                    ...
                    <div className="relative -mt-12 px-6 pb-6">
                        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
                            <div className="flex items-end gap-4">
                                <div
                                    className={cn(
                                        'shadow-soft-lg ring-card flex size-20 items-center justify-center rounded-3xl bg-gradient-to-br text-2xl font-bold text-white ring-4',
                                        gradient,
                                    )}
                                >
                        ...
                        <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                            <Stat icon={CalendarDays} label="Start" value={formatDate(project.start_date)} tone="from-blue-500 to-blue-700" />
                            <Stat icon={CalendarDays} label="Target end" value={formatDate(project.end_date)} tone="from-blue-500 to-cyan-500" />
                            <Stat icon={Users} label="Members" value={project.members.length.toString()} tone="from-slate-500 to-slate-700" />
                        </div>

                        <div className="mt-5 space-y-2">
                            <div className="flex items-center justify-between text-xs">
                                <span className="text-muted-foreground font-bold tracking-[0.14em] uppercase">Overall progress</span>
                ...
                <div className="bg-card ring-border/60 shadow-soft-xs flex w-full flex-wrap gap-1 rounded-2xl p-1 ring-1">
                    {tabs.map((t) => (
                        <button
                            key={t.key}
                ...
                {tab === 'milestones' && (
                    <SoftCard>
                        ...
                        <SoftCardBody>
                            <ol className="before:bg-border relative space-y-4 pl-7 before:absolute before:top-2 before:bottom-2 before:left-3 before:w-px">
                                {project.milestones.length === 0 && <p className="text-muted-foreground text-xs">No milestones yet.</p>}
                                {project.milestones.map((m) => {
                                    ...
                                    return (
                                        <li key={m.id} className="relative">
                                            <span
                                                className={cn(
                                                    'ring-card absolute top-0.5 -left-7 flex size-6 items-center justify-center rounded-full ring-2',
                                                    done
                                            ...
                                            <div
                                                className={cn(
                                                    'bg-muted/40 ring-border/50 rounded-xl p-3 ring-1 transition-all',
                                                    done && 'opacity-70',
                                                )}
                                            >
                                ...
                                {project.members.map((m) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            <div
                                                className={cn(
                                                    'shadow-soft-xs ring-card flex size-11 items-center justify-center rounded-xl bg-gradient-to-br text-sm font-bold text-white ring-2',
                                                    isOwner ? 'from-amber-400 to-orange-500' : 'from-blue-500 to-blue-700',
                                                )}
                                            >
                                                {getInitials(m.name)}
                                            </div>
                                            ...
                                            {isOwner && (
                                                <span className="inline-flex items-center rounded-full bg-amber-50 px-2 py-0.5 text-[10px] font-bold tracking-[0.14em] text-amber-700 uppercase ring-1 ring-amber-200/70 dark:bg-amber-500/10 dark:text-amber-300 dark:ring-amber-500/30">
                                                    Owner
                                                </span>
                                            )}
                                        </Link>
                        ...
                        <SoftCardBody>
                            {canUpload && (
                                <div
                                    ...
                                    onDrop={onDrop}
                                    className={cn(
                                        'mb-4 rounded-2xl border-2 border-dashed p-6 text-center transition-all',
                                        dragActive ? 'border-blue-400 bg-blue-50/60 dark:border-blue-500/40 dark:bg-blue-500/10' : 'border-border',
                                    )}
                                >
                                    <UploadCloud className={cn('mx-auto size-7', dragActive ? 'text-blue-500' : 'text-muted-foreground')} />
                                ...
                                <ul className="divide-border/60 divide-y">
                                    {project.attachments.map((a) => (
                                        <li key={a.id} className="flex items-center gap-3 py-3">
                                            <div className="shadow-soft-xs flex size-10 items-center justify-center rounded-xl bg-gradient-to-br from-blue-500 to-blue-700 text-white">
                                                <File className="size-4" />
                                            </div>
                                            <div className="min-w-0 flex-1">
                                                <p className="truncate text-sm font-semibold">{a.file_name}</p>
                                                <p className="text-muted-foreground truncate text-[11px]">
                                                    {formatBytes(a.file_size)} · {a.uploader?.name ?? '—'} · {new Date(a.created_at).toLocaleString()}
                                                </p>
                                            </div>
                                            <a
                                                href={route('projects.files.download', { project: project.slug, attachment: a.id })}
            ...
            <ConfirmDialog
                ...
                onConfirm={() => {
                    if (confirmDeleteFileId !== null) {
                        router.delete(route('projects.files.destroy', { project: project.slug, attachment: confirmDeleteFileId }), {
                            preserveScroll: true,
                        });
                    }
...
function Stat({ icon: Icon, label, value, tone }: { icon: typeof Users; label: string; value: string; tone: string }) {
    return (
        <div className="bg-card ring-border/50 shadow-soft-xs flex items-center gap-3 rounded-2xl p-3 ring-1">
            <div className={cn('shadow-soft-sm flex size-10 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br text-white', tone)}>
                <Icon className="size-4" />
            </div>
            <div className="min-w-0">
                <p className="text-muted-foreground text-[10px] font-bold tracking-[0.14em] uppercase">{label}</p>
