...
interface AgendaRow { id?: number; title: string; description: string; time_allocation_minutes: number | null }
...
interface Props { meeting: Meeting; people: Person[]; projects: Project[]; kinds: string[] }
...
export default function MeetingEdit({ meeting, people, projects, kinds }: Props) {
    ...
    const togglePerson = (id: number) => {
        ...
        if (exists) setData('participants', data.participants.filter((p) => p.user_id !== id));
    ...
    const removeAgenda = (i: number) => setData('agenda_items', data.agenda_items.filter((_, idx) => idx !== i));
    const setAgenda = (i: number, p: Partial<AgendaRow>) => setData('agenda_items', data.agenda_items.map((a, idx) => idx === i ? { ...a, ...p } : a));
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <form onSubmit={submit} className="flex w-full flex-1 flex-col gap-5 p-4 md:p-6">
                    ...
                    <SoftCardBody className="grid gap-4 md:grid-cols-2">
                        ...
                        <Field label="Kind" error={errors.kind}>
                            <Select value={data.kind} onValueChange={(v) => setData('kind', v)}>
                                ...
                                <SelectContent className="rounded-xl">{kinds.map((k) => <SelectItem key={k} value={k}>{k.replace('_', ' ')}</SelectItem>)}</SelectContent>
                            </Select>
                        </Field>
                ...
                <SoftCard>
                    <SoftCardTitle eyebrow="Agenda" action={<Button type="button" variant="soft" size="sm" className="gap-1.5" onClick={addAgenda}><Plus className="size-3.5" /> Add</Button>}>Agenda</SoftCardTitle>
                    <SoftCardBody>
                ...
                <div className="flex items-center justify-end gap-2">
                    <Button asChild variant="ghost"><Link href={route('meetings.show', meeting.id)}>Cancel</Link></Button>
