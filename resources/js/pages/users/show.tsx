        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex w-full flex-1 flex-col gap-6 p-4 md:p-6">
                ...
                <SoftCard className="overflow-visible">
                    <div className="relative h-40 overflow-hidden rounded-t-2xl bg-gradient-to-br from-blue-600 to-blue-700">
                        <div className="absolute -top-20 -right-20 size-72 rounded-full bg-white/15 blur-3xl" />
                        <div className="absolute -bottom-10 left-1/3 size-56 rounded-full bg-pink-400/30 blur-3xl" />
                        <div
                            className="absolute inset-0"
                            style={{ backgroundImage: 'radial-gradient(rgba(255,255,255,0.18) 1px, transparent 1px)', backgroundSize: '24px 24px' }}
                        />
                    </div>
                    <div className="relative -mt-16 flex flex-col gap-5 px-6 pb-6 md:flex-row md:items-end md:justify-between">
                        <div className="flex flex-col items-start gap-4 md:flex-row md:items-end">
                            <div className="shadow-soft-lg ring-card flex size-28 items-center justify-center rounded-3xl bg-gradient-to-br from-blue-500 to-blue-700 text-3xl font-bold text-white ring-4">
                    ...
                    <div className="border-border/60 grid gap-3 border-t px-6 py-5 sm:grid-cols-2 lg:grid-cols-4">
                        <InfoRow icon={Mail} label="Email" value={user.email} tone="from-blue-500 to-blue-700" />
                        ...
                        <InfoRow icon={Building2} label="Department" value={user.department?.name ?? '—'} tone="from-slate-500 to-slate-700" />
                    </div>
                </SoftCard>

                <div className="bg-card ring-border/60 shadow-soft-xs flex w-fit gap-1 rounded-2xl p-1 ring-1">
                    ...
                    <div className="grid gap-4 lg:grid-cols-3">
                        <SoftCard className="lg:col-span-2">
                            ...
                            <SoftCardBody className="grid gap-3 sm:grid-cols-3">
                                {[
                                    { label: 'Active projects', value: '—', tone: 'text-blue-600' },
                                    { label: 'Open tasks', value: '—', tone: 'text-blue-600' },
                                    { label: 'Completed (30d)', value: '—', tone: 'text-emerald-600' },
                                ].map((stat) => (
                                    <div key={stat.label} className="bg-muted/40 ring-border/50 rounded-xl p-4 ring-1">
                                        <p className="text-muted-foreground text-[10px] font-bold tracking-[0.14em] uppercase">{stat.label}</p>
                    ...
                    <SoftCard>
                        ...
                        <SoftCardBody className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                            {Object.entries(grouped).map(([module, slugs]) => (
                                <div key={module} className="bg-muted/40 ring-border/50 rounded-xl p-4 ring-1">
                                    <p className="text-muted-foreground text-[10px] font-bold tracking-[0.14em] uppercase">{module}</p>
                ...
                {tab === 'activity' && (
                    <SoftCard>
                        ...
                        <SoftCardBody>
                            <ol className="before:bg-border relative space-y-5 pl-6 before:absolute before:top-1.5 before:bottom-1.5 before:left-2 before:w-px">
...
function InfoRow({ icon: Icon, label, value, tone }: { icon: typeof Mail; label: string; value: string; tone: string }) {
    return (
        <div className="flex items-center gap-3">
            <div className={`flex size-10 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br ${tone} shadow-soft-sm text-white`}>
